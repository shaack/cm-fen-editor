...
export class FenEditor_201117 {
    constructor(element, props = {}) {
        ...
        this.setState(EDIT_MODE.move)
        for (const button of this.elements.stateButtons) {
            button.addEventListener("click", () => {
                this.setState(EDIT_MODE[button.dataset.state])
            })
        }
...
class FenEditorState {
    constructor(fen, editMode = EDIT_MODE.move) {
    ...
    set fen(fen) {
        try {
            ...
            console.warn("fen is invalid", fen)
            this.valid = false
        }
    }
}